...
export class SecondaryTokenURIUpdated extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    ...
    assert(
      id != null,
      "Cannot save SecondaryTokenURIUpdated entity without an ID"
    );
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type SecondaryTokenURIUpdated must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("SecondaryTokenURIUpdated", id.toBytes().toHexString(), this);
    }
  }

  static load(id: Bytes): SecondaryTokenURIUpdated | null {
    return changetype<SecondaryTokenURIUpdated | null>(
      store.get("SecondaryTokenURIUpdated", id.toHexString())
    );
  }
  ...
  get oldUri(): string {
    let value = this.get("oldUri");
    return value!.toString();
  }

  set oldUri(value: string) {
    this.set("oldUri", Value.fromString(value));
  }
